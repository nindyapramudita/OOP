...
class Pendataan_sepatu: #template/blueprint pendataan sepatu
    ...
    def tampilkan_data(self): #method tampilkan data sepatu yang ada
        if self.data_sepatu: #jika list sepatu
            for idx, sepatu in enumerate(self.data_sepatu, start=1): #buat id sepatunya biar mudah dicari
                print(f"ID: {idx}, Nama: {sepatu.nama}, Warna: {sepatu.warna}, Ukuran: {sepatu.ukuran}, Harga: {sepatu.harga}, Stock: {sepatu.stock}, Terjual: {sepatu.terjual}\n")#tampilan datanya
        else:
...
def menu(): #funtion untuk menu dan inputannya
