...
class Pendataan_sepatu:
    ...
    def fibonacci_search(self, key, search_key):
        n = self.count_nodes()
        fib_m_minus_2 = 0
        fib_m_minus_1 = 1
        ...
        while fib_m > 1:
            ...
            if search_key == 'id':
                if int(self.get_node(i).sepatu.id) < int(key):
                    fib_m = fib_m_minus_1
                    fib_m_minus_1 = fib_m_minus_2
                    fib_m_minus_2 = fib_m - fib_m_minus_1
                    offset = i
                elif int(self.get_node(i).sepatu.id) > int(key):
                    fib_m = fib_m_minus_2
                    fib_m_minus_1 -= fib_m_minus_2
                    fib_m_minus_2 = fib_m - fib_m_minus_1
                else:
                    return i
            elif search_key == 'nama':
                if self.get_node(i).sepatu.nama.lower() < key.lower():
                    fib_m = fib_m_minus_1
                    fib_m_minus_1 = fib_m_minus_2
                    fib_m_minus_2 = fib_m - fib_m_minus_1
                    offset = i
                elif self.get_node(i).sepatu.nama.lower() > key.lower():
                    fib_m = fib_m_minus_2
                    fib_m_minus_1 -= fib_m_minus_2
                    fib_m_minus_2 = fib_m - fib_m_minus_1
                else:
        ...
        if fib_m_minus_1 and self.get_node(min(offset + 1, n - 1)).sepatu.id == key:
            return min(offset + 1, n - 1)
    ...
    def search_data(self, key, search_key):
        index = self.fibonacci_search(key, search_key)
        if index != -1:
            sepatu = self.get_node(index).sepatu
            print(f"Data ditemukan!\nID: {sepatu.id}, Nama: {sepatu.nama}, Warna: {sepatu.warna}, Ukuran: {sepatu.ukuran}, Harga: {sepatu.harga}, Stock: {sepatu.stock}, Terjual: {sepatu.terjual}\n")
        else:
...
def menu_tampilan():
    ...
    while True:
        ...
        if pilihan == "1":
            ...
            print("\n=======SEARCH DATA SEPATU========")
            search_key = input("Pilih kriteria pencarian (id/nama): ")
            if search_key not in ['id', 'nama']:
                print("Kriteria pencarian tidak valid.")
            else:
                key = input("Masukkan nilai yang ingin dicari: ")
                pendataan.search_data(key, search_key)
